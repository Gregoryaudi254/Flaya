...
import { getFirestore, collection, query, orderBy, limit, getDocs,serverTimestamp, setDoc, doc,startAfter, startAt } from 'firebase/firestore';
...
const interactingusers = React.memo(({ isVisible, onClose, info}) => {
  ...
  const updateId = info.id;
  const postcoordinate = info.coordinate;
  ...
  const fetchInitialUsers = useCallback(async () => {
    ...
    try {
      console.log(updateId +"and "+userinfo.uid)
      const collectionsRef = collection(db, `users/${userinfo.uid}/updates/${updateId}/collections`);
      const q = query(collectionsRef, orderBy('createdAt', 'desc'), limit(11));
  ...
  const fetchMoreUsers = useCallback(async (lastUser) => {
    ...
    try {
      const collectionsRef = collection(db, `users/${userinfo.uid}/updates/${updateId}/collections`);
      const q = query(collectionsRef, orderBy('createdAt', 'desc'), startAfter(lastUser.createdAt), limit(11));
      const querySnapshot = await getDocs(q);
