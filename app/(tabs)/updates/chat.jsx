...
const chat = () => {
    ...
    const setTypingStatus = async (status) => {
      if (userInfo && oppusertyping !== null) {
        try{
          const oppMessageRef = doc(db,`users/${oppUser.uid}/messages/${userInfo.uid}`);
          await updateDoc(oppMessageRef, {typing:status || isTyping})
        }catch(e){}  
      }
    }
                  ...
                  if (chatIndex > -1) { // Message is already in the list
                      ...
                      if (type === 'added') {
                        ...
                        // New message not in the list, add it to the top
                         return [ ...prevChats, {...messages,status:'sent'}];
    ...
    const listenTypingStatus = async (callback) => {
      ...
      const q = query(chatRef, where('id','==', oppUser.uid.trim()),);
    ...
    useEffect(() => {
      ...
      (async () => {
          unsubscribe = await listenTypingStatus((newMessage) => {
            ...
            console.log("photo "+newMessage.photo);
    ...
    const listenOppuserOnlineStatus = async (callback) => {
      ...
      const q = query(userRef, where('uid','==', oppUser.uid),);
    ...
    useEffect(() => {
      ...
      // Call the async function and set the unsubscribe function
      (async () => {
          unsubscribe = await listenOppuserOnlineStatus((data) => {

            console.log(JSON.stringify(data)+" data")
            const online = data.isonline;
            setoppuseronline(online);

            setoppuserlastseen(data.lastseen);
          ...
          headerLeft: () => (
            ...
            <View style={{flexDirection:'row',alignItems:'center'}}>
                ...
                <TouchableOpacity  onPress={handleHeaderLeftPress}>
                    ...
                    style={{ height: 20, tintColor: 'white', alignSelf: 'flex-end' }}
                    />
               ...
               <TouchableOpacity>
                ...
                >
                    <Image
                    resizeMode="cover"
                    source={oppuserprofile.profilephoto ?{uri: oppuserprofile.profilephoto} :require('@/assets/icons/user.png')}
                    style={[styles.profileImage, !hasStories && {borderWidth:0}] }
                    />
                ...
                <View style={{marginStart:10}}>
                  ...
                  <Text style={{fontSize:13,color:'white'}}>{oppUser.username}</Text>

                  {oppuserlastseen !== null && <Text style={{fontSize:13,color:'gray'}}>{(oppusertyping!== null && oppusertyping) ? 'typing':(oppuseronline !== null && oppuseronline) ? 'online' : timeAgo(oppuserlastseen)}</Text>}
        ...
        data={chats}/>
          ...
          {  isReplying &&
            <View style={{alignItems:'center'}}>
              ...
              <View style={{width:'100%',height:1,backgroundColor:'white',marginBottom:10}}/>
              ...
              <View >
                ...
                <Text style={{fontSize:13,color:'white',marginBottom:5}}>{selectedItem.senderid === userInfo.uid ? userInfo.username:oppUser.username}</Text>
                ...
                { 
                  ...
                  />):(
                    <View style={{flexDirection:'row'}}>
                      ...
                      <Image
                        style={{width:25,height:25,borderRadius:3,marginBottom:5,tintColor:'white'}}
              ...
              <TouchableOpacity onPress={handleReply}>
                <Image
                      style={{width:15,height:15,borderRadius:3,tintColor:'white'}}
         ...
         {
          (isChatAccepted !== null && isChatAccepted && !isrequest) ? 
            <View style={styles.container}>
              ...
              value={text}
              placeholder='Write something..' style={styles.textInput}/>
            ...
            (isChatAccepted !== null && !isrequest) ?

            <View style={{height:60,width:'100%'}}>

              <View style={{width:'100%',height:0.8,backgroundColor:"white"}}/>

              <Text style={{fontSize:20,color:'gray',alignSelf:'center',marginTop:10}}>Request sent!</Text>
        ...
        <CustomDialog onclose={handleCloseDialog}  isVisible={showDialog}>
...

          {
            ...
            <View  style={styles.modalSelection}>
              ...
              <Text style={{color:'red',margin:5,fontSize:20}}>Delete for you</Text>
